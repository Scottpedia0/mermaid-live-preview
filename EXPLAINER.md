# Mermaid Live Preview — What this is and why it exists
...
You right-click a Mermaid block, the preview opens, you see the rendered diagram, you edit the Markdown, the diagram shifts to match. When you need to jump back to a specific block in the source, you can do that from the preview.
...
Mermaid syntax costs slightly more tokens than a plain bullet list or YAML would. A large diagram might add 1,000-2,000 tokens. In conversations running against 100k-1M token context windows, this is negligible.
...
When you want your agent to include visual workflows in shared documents, add an instruction like this to your agent's system prompt, CLAUDE.md, AGENTS.md, or equivalent configuration file:
...
This gives the human a fast way to visually audit what the agent defined, while keeping the Markdown file as the single source of truth that both sides work from.
